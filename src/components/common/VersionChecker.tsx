  ...
  const fetchVersion = async () => {
    try {
      ...
      if (!contentType || !contentType.includes("application/json")) {
        console.warn(
          "version.json returned non-JSON content, skipping version check",
        );
        return null;
      }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-[9999]">
      <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full mx-4 p-6 text-center">
        ...
        <p className="text-gray-700 mb-6">
          Uma nova versão da aplicação está disponível. Por favor, recarregue a
          página para obter as últimas atualizações.
        </p>
        <button
