...
const Header: React.FC<HeaderProps> = ({
  ...
  const SidebarContent = ({ isMobile = false }: { isMobile?: boolean }) => {
    return (
      <div className={`flex flex-col h-full ${isCollapsed && !isMobile ? 'items-center' : ''}`}>
        {/* Header */}
        <div className={`p-4 border-b border-gray-100 flex items-center ${isCollapsed && !isMobile ? 'justify-center' : 'justify-between'}`}>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 capitalize">{user?.type === "manager" ? "Gerente" : "Cobrador"}</p>
                  </div>
                </div>
          ...
          <nav className="space-y-1">
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  <Icon className={`h-5 w-5 ${!isCollapsed || isMobile ? 'mr-3' : ''} ${activeTab === tab.id ? "text-blue-600" : "text-gray-400"}`} />
                  {(showDesktopText || isMobile) && <span className={`font-medium`}>{tab.name}</span>}
                  {(showDesktopText || isMobile) && tab.id === "visit-tracking" && pendingCancellations > 0 && (
  ...
  return (
    <>
      ...
      <div className="lg:hidden absolute top-0 right-0 z-30 p-4">
        <button onClick={() => setIsMobileMenuOpen(true)} className="p-2 text-gray-500 bg-white rounded-md">
      ...
      {/* Desktop Sidebar */}
      <aside className={`hidden lg:block bg-white shadow-md flex-shrink-0 transition-all duration-300 z-40 ${isCollapsed ? 'w-20' : 'w-72'}`}>
        <SidebarContent />
      </aside>
    </>
  );
