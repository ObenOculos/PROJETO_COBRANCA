...
const VisitCancellationApproval: React.FC<VisitCancellationApprovalProps> = ({ onClose }) => {
  const { 
  ...
  const [cancellationHistory, setCancellationHistory] = useState<ScheduledVisit[]>([]);
        ...
        <div className="bg-gradient-to-r from-red-600 to-pink-600 text-white px-4 lg:px-6 py-4 rounded-t-xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <div>
                <h2 className="text-lg lg:text-xl font-bold">Aprovação de Cancelamentos</h2>
                <p className="text-red-100 text-sm lg:text-base">Gerencie solicitações de cancelamento de visitas</p>
              </div>
            </div>
            {onClose && (
              <button
        ...
        <div className="p-4 lg:p-6">
          {activeTab === 'pending' ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-500 text-lg">Nenhuma solicitação de cancelamento pendente</p>
            ...
            <div className="space-y-4">
              ...
              {pendingRequests.map((request) => (
                <div key={request.id} className="border border-red-200 rounded-lg p-3 lg:p-4 bg-red-50">
            ...
            <>
              {cancellationHistory.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-500 text-lg">Nenhum cancelamento nos últimos 30 dias</p>
                  <p className="text-gray-400 text-sm mt-2">O histórico aparecerá aqui conforme as decisões forem tomadas</p>
                </div>
              ) : (
                <div className="space-y-4">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-base lg:text-lg font-semibold text-gray-900">
                      Histórico dos últimos 30 dias ({cancellationHistory.length} {cancellationHistory.length === 1 ? 'decisão' : 'decisões'})
                    </h3>
                  </div>

                  {cancellationHistory.map((visit) => {
                    const isApproved = visit.status === 'cancelada' && visit.cancellationApprovedBy;
                    const isRejected = visit.status === 'agendada' && visit.cancellationRejectedBy;
                    const decisionDate = isApproved ? visit.cancellationApprovedAt : visit.cancellationRejectedAt;
                    const decisionBy = isApproved ? visit.cancellationApprovedBy : visit.cancellationRejectedBy;
                    ...
                    return (
                      ...
                      >
                        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between space-y-3 lg:space-y-0">
                          <div className="flex-1">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-3 mb-3">
                              <div className="font-semibold text-gray-900 mb-1 sm:mb-0">{visit.clientName}</div>
                              <span className={`px-2 py-1 rounded-full text-xs w-fit ${
                                isApproved 
                                  ? 'bg-green-100 text-green-800' 
                                  : 'bg-red-100 text-red-800'
                              }`}>
                                {isApproved ? 'Cancelamento Aprovado' : 'Cancelamento Rejeitado'}
                              </span>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 lg:gap-4 text-sm text-gray-600 mb-3">
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="flex items-center">
                                  <User className="h-4 w-4 mr-2" />
                                  Decidido por: {getCollectorName(decisionBy || '')}
                                </div>
                              </div>
                            </div>
                            ...
                            {isRejected && visit.cancellationRejectionReason && (
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full mx-4">
            ...
            <div className="px-4 lg:px-6 py-4 border-t border-gray-200 flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3">
              ...
              >
                ...
                <span className="ml-2">{loading ? 'Processando...' : (approvalAction === 'approve' ? 'Aprovar' : 'Rejeitar')}</span>
              </button>
            </div>
          </div>
        </div>
