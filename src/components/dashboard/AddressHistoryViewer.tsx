...
const AddressHistoryViewer: React.FC<AddressHistoryViewerProps> = ({ clientDocument }) => {
      ...
      <div className="space-y-4">
        ...
        <div className="bg-gray-50 p-4 rounded-2xl space-y-4 text-sm border border-gray-200">
          <div className="flex items-start">
            ...
            {addressLoading ? (
              <div className="text-gray-800 animate-pulse">Carregando endereço...</div>
            ) : currentAddress ? (
              <div className="text-gray-800">
                ...
                {currentAddress.cep && <p className="text-xs">{currentAddress.cep}</p>}
                 <p className="text-xs text-gray-400 mt-1">
                    (Atualizado em: {new Date(currentAddress.created_at).toLocaleDateString('pt-BR')})
                 </p>
              </div>
            ) : (
              <div className="text-gray-800">
                <p>Nenhum endereço cadastrado.</p>
                <p className="text-xs text-gray-500">Clique em "Atualizar Endereço" para adicionar um.</p>
              </div>
            )}
          </div>
          ...
          {historicalAddresses.length > 0 && (
            <div className="pt-4 mt-4 border-t border-gray-200">
              <h4 className="text-sm font-semibold text-gray-700 mb-2">Histórico de Endereços Anteriores</h4>
              <div className="space-y-3 max-h-24 overflow-y-auto pr-2">
                {historicalAddresses.map((addr) => (
                  <div key={addr.id} className="text-xs text-gray-500 border-l-2 pl-2 border-gray-300">
                    <p>
                    ...
                    <p className="text-gray-400">
                      (Registrado em: {new Date(addr.created_at).toLocaleDateString('pt-BR')})
                    </p>
                  </div>
                ))}
              </div>
