...
const SalePaymentModal: React.FC<SalePaymentModalProps> = ({ 
  ...
  const saleBalance = calculateSaleBalance(saleGroup.saleNumber, saleGroup.clientDocument);
    ...
    for (const installment of pendingInstallments) {
      ...
      if (appliedAmount > 0) {
        distribution.push({
          ...
          willBeComplete: (installment.valor_recebido + appliedAmount) >= installment.valor_original
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Notificação de erro
      const notification = document.createElement('div');
      notification.className = 'fixed top-4 right-4 bg-red-500 text-white px-4 py-2 rounded-lg shadow-lg z-50 flex items-center';
      notification.innerHTML = `
        <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 mb-6 border border-blue-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="text-center p-4 bg-white rounded-lg shadow-sm">
                ...
                <div className="text-sm text-gray-600">Já Recebido</div>
              </div>
            ...
            <div className="mt-4 flex justify-center">
              <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                ...
                {saleBalance.status === 'fully_paid' && <CheckCircle className="h-4 w-4 mr-1" />}
                {saleBalance.status === 'partially_paid' && <Clock className="h-4 w-4 mr-1" />}
                {saleBalance.status === 'pending' && <AlertCircle className="h-4 w-4 mr-1" />}
            ...
            <div className="space-y-6">
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center px-4 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-semibold"
                >
                  {loading ? (
            ...
            <div className="space-y-4">
              ...
              {hasValidAmount && showPreview ? (
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {distribution.length > 0 ? (
                        ...
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            ...
                            <div className="text-sm text-gray-600">
                              Venc: {new Date(item.installment.data_vencimento || '').toLocaleDateString('pt-BR')}
                            </div>
                          </div>
                          
                          <div className="text-right">
                            <div className="text-sm text-gray-600">
                              Aplicar: <span className="font-semibold text-green-600">
                                {formatCurrency(item.appliedAmount)}
                              </span>
                            </div>
                            <div className="text-xs text-gray-500">
                              Total: {formatCurrency(item.newTotal)} / {formatCurrency(item.installment.valor_original)}
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p>Todas as parcelas já estão quitadas</p>
                    </div>
                  )}
                  ...
                  {parseFloat(paymentAmount) > saleBalance.remainingBalance && (
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                      <div className="flex items-center text-yellow-800">
                        ...
                        <span className="font-medium">Valor maior que o saldo</span>
                      </div>
                      <p className="text-yellow-700 text-sm mt-1">
                        O valor excede o saldo devedor em {formatCurrency(parseFloat(paymentAmount) - saleBalance.remainingBalance)}
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p>Digite um valor para ver a distribuição</p>
                </div>
              )}
            </div>
          </div>
