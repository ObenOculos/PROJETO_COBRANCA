...
const SalePaymentModal: React.FC<SalePaymentModalProps> = memo(
  ({ saleGroup, onClose, onSuccess }) => {
    ...
    const [autoGeneratedNote, setAutoGeneratedNote] = useState("");
    ...
    useEffect(() => {
      if (withDiscount) {
        ...
        if (amount > 0) {
          const discountAmount = saleBalance.remainingBalance - amount;
          if (discountAmount >= 0) {
            setAutoGeneratedNote(
              `Pagamento com desconto. Valor real pago: ${formatCurrency(
                amount,
    ...
    }, [withDiscount, paymentAmount, saleBalance.remainingBalance]);
    ...
    const handleProcessPayment = async () => {
      if (!user) {
        showErrorNotification(new Error("Usuário não identificado."));
        return false;
      }
      ...
      if (isNaN(amount) || amount <= 0) {
        showErrorNotification(
          new Error("Digite um valor válido para o pagamento."),
        );
        return false;
      }
      ...
      try {
        ...
        const discountAmount = withDiscount
          ? saleBalance.remainingBalance - amount
          : 0;
        ...
        const finalNotes = `${autoGeneratedNote} ${notes.trim()}`.trim();
          ...
          <div className="overflow-y-auto max-h-[calc(95vh-180px)] sm:max-h-[calc(90vh-180px)]">
            <form onSubmit={handleSubmit} className="p-4 sm:p-6">
              ...
              <div className="mb-6">
                ...
                />
                {autoGeneratedNote && (
                  <div className="mt-2 p-3 bg-gray-100 border border-gray-200 rounded-2xl text-sm text-gray-600">
                    <p>
                      <strong>Nota adicionada automaticamente:</strong>{" "}
                      {autoGeneratedNote}
                    </p>
                  </div>
                )}
