...
const CollectionTable: React.FC<CollectionTableProps> = ({ collections, userType, showGrouped = true, collectorId }) => {
  ...
  const [sortField, setSortField] = useState<'cliente' | 'valor' | 'vencimento' | 'status' | 'cidade'>('vencimento');
  ...
  const expandAll = () => {
    const allClientDocuments = new Set(paginatedSalesGroups.map(group => group.document));
  ...
  const totalItems = showGrouped ? filteredClientGroups.length : filteredAndGroupedSales.length;
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="px-4 py-4 border-b border-gray-200">
            <h2 className="text-lg font-semibold text-gray-900">
              {userType === 'manager' ? 'Clientes e Cobranças' : 'Meus Clientes'}
            </h2>
            <div className="flex justify-between items-center mt-1">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                <p className="text-sm text-gray-600">
                  {filteredClientGroups.length} cliente{filteredClientGroups.length !== 1 ? 's' : ''} com cobranças
                </p>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex-shrink-0 text-right">
                      ...
                      <div className="text-xs text-gray-500">
                        {clientGroup.sales.length} venda{clientGroup.sales.length !== 1 ? 's' : ''}
                      </div>
                    </div>
                  </div>
          ...
          <div className="bg-gradient-to-r from-purple-600 to-indigo-600 px-4 sm:px-6 py-4 rounded-b-xl">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="text-sm text-white text-center sm:text-left">
                <span className="font-semibold">Página {currentPage}</span> de <span className="font-semibold">{totalPages}</span>
                <span className="hidden sm:inline text-purple-100 ml-2">• {totalItems} clientes total</span>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200">
            ...
            <div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mt-1">
                <p className="text-sm sm:text-base text-gray-600">
                  {filteredAndGroupedSales.length} cliente{filteredAndGroupedSales.length !== 1 ? 's' : ''} com vendas
                </p>
                <div className="flex items-center gap-4">
                  {totalPages > 1 && (
                    <p className="text-sm text-gray-500">
          ...
          {userType === 'collector' && showFilters && (
            <div className="mt-6 p-4 bg-gradient-to-r from-gray-50 to-blue-50 rounded-lg border border-gray-200 animate-in slide-in-from-top-2 duration-200">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Status da Cobrança</label>
                  <select
                    value={statusFilter}
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mt-4 p-4 bg-gradient-to-r from-purple-50 to-indigo-50 rounded-lg border border-purple-200">
              <div className="flex items-center gap-4">
                <span className="text-sm text-gray-600 font-medium">
                  Mostrando <span className="text-purple-600 font-bold">{startItem}</span> a <span className="text-purple-600 font-bold">{endItem}</span> de <span className="text-purple-600 font-bold">{totalItems}</span> clientes
                </span>
                <div className="flex items-center space-x-2 text-sm text-gray-500">
                  <TrendingUp className="h-4 w-4 text-purple-500" />
                  <span>Página <span className="font-semibold text-gray-700">{currentPage}</span> de <span className="font-semibold text-gray-700">{totalPages}</span></span>
                </div>
              </div>
        ...
        <div className="divide-y divide-gray-100">
          {paginatedSalesGroups.map((clientGroup) => (
            <div key={clientGroup.document} className="bg-white hover:shadow-sm transition-all duration-200">
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-6 flex-1">
                            ...
                            <div className="flex-shrink-0 text-center">
                              <div className={`text-sm font-bold transition-colors ${
                                ...
                                {sale.pendingValue > 0 ? formatCurrency(sale.pendingValue) : 'Quitado'}
                              </div>
                              <div className={`text-xs mt-1 transition-colors ${
                                ...
                                {sale.pendingValue > 0 ? 'Pendente' : 'Finalizado'}
                              </div>
                            </div>
                            ...
                            <div className="flex-shrink-0">
                              <div className="flex items-center justify-center">
                                <div className="mr-2 group-hover:scale-110 transition-transform">
                                  {getStatusIcon(displayStatus === 'Quitado' ? 'recebido' : 'pendente')}
                                </div>
                                <span className={`inline-flex px-4 py-2 text-xs font-bold rounded-full shadow-md group-hover:shadow-lg transition-all duration-200 ${statusColor} group-hover:scale-105`}>
                                  {displayStatus}
                                </span>
                              </div>
                            </div>
                              ...
                              <>
                                ...
                                <button
                                  onClick={(e) => {
                                    ...
                                    const clientGroupForModal = clientGroups.find(cg => cg.document === clientGroup.document);
        ...
        {filteredAndGroupedSales.length === 0 && (
          <div className="py-16 text-center">
            <div className="text-gray-500">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">Nenhum cliente com vendas encontrado</h3>
      ...
      {totalPages > 1 && (
        <div className="bg-gradient-to-r from-purple-600 to-indigo-600 border-t border-gray-200 px-4 sm:px-6 py-4 rounded-b-xl">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="text-sm text-white text-center sm:text-left">
              <span className="font-semibold">Página {currentPage}</span> de <span className="font-semibold">{totalPages}</span>
              <span className="hidden sm:inline text-purple-100"> • {totalItems} clientes total</span>
              <div className="sm:hidden text-xs text-purple-100 mt-1">
                {totalItems} clientes total
              </div>
            </div>
