...
const EnhancedPerformanceChart: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<'conversionRate' | 'receivedAmount' | 'totalSales'>('conversionRate');
  ...
  const enhancedPerformance = useMemo((): EnhancedCollectorPerformance[] => {
    ...
    return collectors.map(collector => {
      const collectorCollections = collections.filter(c => c.user_id === collector.id);
      ...
      const completedSales = salesArray.filter(s => s.status === 'pago').length;
      const partialSales = salesArray.filter(s => s.status === 'parcial').length;
      const pendingSales = salesArray.filter(s => s.status === 'pendente').length;
  ...
  const teamStats = useMemo(() => {
    const totalSales = enhancedPerformance.reduce((sum, p) => sum + p.totalSales, 0);
    const totalReceived = enhancedPerformance.reduce((sum, p) => sum + p.receivedAmount, 0);
    const totalAmount = enhancedPerformance.reduce((sum, p) => sum + p.totalAmount, 0);
    const avgConversionRate = enhancedPerformance.length > 0 
      ? enhancedPerformance.reduce((sum, p) => sum + p.conversionRate, 0) / enhancedPerformance.length
      : 0;
    const topPerformer = enhancedPerformance.length > 0 
    ...
    <div className="space-y-6">
        ...
        <div className="p-4 lg:p-6 border-b border-gray-200">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div className="min-w-0">
              ...
              <p className="text-gray-600 mt-1 text-sm lg:text-base">Desempenho detalhado dos cobradores por vendas</p>
            </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-6">
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-4 lg:p-6 rounded-xl border border-blue-200">
          <div className="flex items-center justify-between">
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-blue-700">Total de Vendas</p>
              <p className="text-2xl lg:text-3xl font-bold text-blue-900 truncate">{teamStats.totalSales}</p>
            </div>
        ...
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 p-4 lg:p-6 rounded-xl border border-green-200">
          <div className="flex items-center justify-between">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-2xl lg:text-3xl font-bold text-green-900 truncate">{formatCurrency(teamStats.totalReceived)}</p>
            </div>
        ...
        <div className="bg-gradient-to-br from-purple-50 to-violet-50 p-4 lg:p-6 rounded-xl border border-purple-200">
          <div className="flex items-center justify-between">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-2xl lg:text-3xl font-bold text-purple-900">{teamStats.avgConversionRate.toFixed(1)}%</p>
            </div>
        ...
        <div className="bg-gradient-to-br from-orange-50 to-red-50 p-4 lg:p-6 rounded-xl border border-orange-200">
          <div className="flex items-center justify-between">
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-orange-700">Eficiência do Time</p>
              <p className="text-2xl lg:text-3xl font-bold text-orange-900">{teamStats.teamEfficiency.toFixed(1)}%</p>
            </div>
        ...
        <div className="p-4 lg:p-6 border-b border-gray-200 bg-white rounded-xl">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Taxa Mínima (%)</label>
                <div className="flex items-center">
      ...
      <div className="space-y-4">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
          <h3 className="text-base lg:text-lg font-semibold text-gray-900">
            Desempenho Individual ({filteredAndSortedPerformance.length} cobradores)
          </h3>
          {teamStats.topPerformer && (
            <div className="flex items-center text-sm text-green-600">
              ...
              <span className="truncate">
                ...
                {teamStats.topPerformer.collectorName} ({teamStats.topPerformer.conversionRate.toFixed(1)}%)
              </span>
            </div>
          )}
        </div>
        ...
        {filteredAndSortedPerformance.map((collector, index) => {
          ...
          const isTopPerformer = index === 0 && sortBy === 'conversionRate' && sortOrder === 'desc';

          return (
            <div 
              key={collector.collectorId} 
              className={`bg-white rounded-xl shadow-sm border transition-all duration-200 hover:shadow-md ${
                isTopPerformer ? 'border-green-300 bg-green-50' : 'border-gray-200'
              }`}
            >
              <div className="p-4 lg:p-6">
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex items-start gap-3">
                    <div className={`h-10 w-10 lg:h-12 lg:w-12 rounded-full flex items-center justify-center flex-shrink-0 ${
                      isTopPerformer ? 'bg-green-100' : 'bg-blue-100'
                    }`}>
                      {isTopPerformer ? (
                        <Award className={`h-5 w-5 lg:h-6 lg:w-6 ${isTopPerformer ? 'text-green-600' : 'text-blue-600'}`} />
                      ) : (
                        <Users className={`h-5 w-5 lg:h-6 lg:w-6 ${isTopPerformer ? 'text-green-600' : 'text-blue-600'}`} />
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="text-base lg:text-lg font-semibold text-gray-900 flex items-center flex-wrap gap-2">
                        <span className="truncate">{collector.collectorName}</span>
                        {isTopPerformer && <Award className="h-4 w-4 text-green-600 flex-shrink-0" />}
                      </h4>
                      <p className="text-sm text-gray-600 mt-1">
                        {collector.totalSales} vendas • {collector.clientsCount} clientes
                      </p>
                      ...
                      <div className="mt-2 flex items-center justify-between">
                        ...
                        <button
                          onClick={() => toggleCardExpansion(collector.collectorId)}
                  ...
                  <div className="mb-4">
                    <div className="flex justify-between text-sm text-gray-600 mb-2">
                      ...
                      <span>{collector.completedSales} de {collector.totalSales} completas</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-3">
                      <div
                        className={`h-3 rounded-full transition-all duration-500 ${
                          collector.conversionRate >= 70 ? 'bg-green-500' :
                          collector.conversionRate >= 40 ? 'bg-yellow-500' : 'bg-red-500'
                        }`}
                  ...
                  {isExpanded && (
                    <div className="mt-4 lg:mt-6 pt-4 lg:pt-6 border-t border-gray-200">
                      <div className="space-y-6 lg:space-y-0 lg:grid lg:grid-cols-3 lg:gap-6">
                        <div>
                          <h5 className="font-medium text-gray-900 mb-3">Status das Vendas</h5>
                          <div className="space-y-2">
                            <div className="flex justify-between p-2 bg-gray-50 rounded">
                              <span className="text-sm text-gray-600">Pagas Completamente</span>
                              <span className="text-sm font-medium text-green-600">{collector.completedSales}</span>
                            </div>
                            <div className="flex justify-between p-2 bg-gray-50 rounded">
                              <span className="text-sm text-gray-600">Pagamento Parcial</span>
                              <span className="text-sm font-medium text-yellow-600">{collector.partialSales}</span>
                            </div>
                            <div className="flex justify-between p-2 bg-gray-50 rounded">
                              <span className="text-sm text-gray-600">Pendentes</span>
                              <span className="text-sm font-medium text-red-600">{collector.pendingSales}</span>
                            </div>
                          </div>
                        </div>

                        <div>
                          <h5 className="font-medium text-gray-900 mb-3">Valores Financeiros</h5>
                          <div className="space-y-2">
                            <div className="flex justify-between p-2 bg-gray-50 rounded">
                              <span className="text-sm text-gray-600">Total Atribuído</span>
                              <span className="text-sm font-medium text-gray-900 truncate ml-2">{formatCurrency(collector.totalAmount)}</span>
                            </div>
                            <div className="flex justify-between p-2 bg-gray-50 rounded">
                            ...
                            <div className="flex justify-between p-2 bg-gray-50 rounded">
                              <span className="text-sm text-gray-600">% do Total Recebido</span>
                        ...
                        <div>
                          <h5 className="font-medium text-gray-900 mb-3">Ranking</h5>
                          <div className="space-y-2">
                            <div className="flex justify-between p-2 bg-gray-50 rounded">
                              <span className="text-sm text-gray-600">Taxa de Conversão</span>
                              <span className="text-sm font-medium text-blue-600">
                                #{enhancedPerformance.sort((a, b) => b.conversionRate - a.conversionRate).findIndex(p => p.collectorId === collector.collectorId) + 1}
                              </span>
                            </div>
                            <div className="flex justify-between p-2 bg-gray-50 rounded">
                              <span className="text-sm text-gray-600">Valor Recebido</span>
                              <span className="text-sm font-medium text-blue-600">
                                #{enhancedPerformance.sort((a, b) => b.receivedAmount - a.receivedAmount).findIndex(p => p.collectorId === collector.collectorId) + 1}
                              </span>
                            </div>
                            <div className="flex justify-between p-2 bg-gray-50 rounded">
                              <span className="text-sm text-gray-600">Número de Vendas</span>
                              <span className="text-sm font-medium text-blue-600">
                                #{enhancedPerformance.sort((a, b) => b.totalSales - a.totalSales).findIndex(p => p.collectorId === collector.collectorId) + 1}
                              </span>
                            </div>
                          </div>
                        </div>
      ...
      {filteredAndSortedPerformance.length === 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">Nenhum cobrador encontrado</h3>
