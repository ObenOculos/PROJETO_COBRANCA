...
const EnhancedStoreManagement: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<'storeName' | 'conversionRate' | 'totalAmount'>('storeName');
  ...
  const storeStats = useMemo((): StoreStats[] => {
    ...
    availableStores.forEach(storeName => {
      ...
      const storeCollections = collections.filter(c => c.nome_da_loja === storeName);
      
      // Find who is actually working on this store (from collections)
      const workingCollectors = new Set(storeCollections.map(c => c.user_id).filter(Boolean));
      
      // Find formal assignment (from collectorStores)
      const assignment = collectorStores.find(cs => cs.storeName === storeName);
      ...
      if (!assignedCollector && workingCollectors.size === 1) {
        ...
        storeCollections.forEach(c => {
          if (c.user_id) {
            collectorCounts.set(c.user_id, (collectorCounts.get(c.user_id) || 0) + 1);
          }
        });
        const mostActiveCollector = Array.from(collectorCounts.entries())
          .sort((a, b) => b[1] - a[1])[0];
  ...
  const overviewStats = useMemo(() => {
    ...
    const totalSales = storeStats.reduce((sum, s) => sum + s.totalCollections, 0);
    const totalRevenue = storeStats.reduce((sum, s) => sum + s.receivedAmount, 0);
    const avgConversionRate = storeStats.length > 0 
      ? storeStats.reduce((sum, s) => sum + s.conversionRate, 0) / storeStats.length 
      : 0;
    ...
    <div className="space-y-6">
        ...
        <div className="p-4 lg:p-6 border-b border-gray-200">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div className="min-w-0">
              ...
              <p className="text-gray-600 mt-1 text-sm lg:text-base">Monitore performance e status das lojas</p>
            </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-6">
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-4 lg:p-6 rounded-xl border border-blue-200">
          <div className="flex items-center justify-between">
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-blue-700">Total de Lojas</p>
              <p className="text-2xl lg:text-3xl font-bold text-blue-900 truncate">{overviewStats.totalStores}</p>
            </div>
        ...
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 p-4 lg:p-6 rounded-xl border border-green-200">
          <div className="flex items-center justify-between">
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-green-700">Lojas Atribuídas</p>
              <p className="text-2xl lg:text-3xl font-bold text-green-900">{overviewStats.assignedStores}</p>
            </div>
        ...
        <div className="bg-gradient-to-br from-purple-50 to-violet-50 p-4 lg:p-6 rounded-xl border border-purple-200">
          <div className="flex items-center justify-between">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-2xl lg:text-3xl font-bold text-purple-900 truncate">{overviewStats.avgConversionRate.toFixed(1)}%</p>
            </div>
        ...
        <div className="bg-gradient-to-br from-orange-50 to-red-50 p-4 lg:p-6 rounded-xl border border-orange-200">
          <div className="flex items-center justify-between">
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-orange-700">Receita Total</p>
              <p className="text-2xl lg:text-3xl font-bold text-orange-900 truncate">{formatCurrency(overviewStats.totalRevenue)}</p>
            </div>
        ...
        <div className="p-4 lg:p-6 border-b border-gray-200 bg-white rounded-xl">
          <div className="space-y-4">
            <div>
              <label htmlFor="store-search" className="block text-sm font-medium text-gray-700 mb-2">Buscar</label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label htmlFor="sort-by" className="block text-sm font-medium text-gray-700 mb-2">Ordenar Por</label>
                <select
                  id="sort-by"
                  name="sortBy"
                  value={sortBy}
          ...
          <div className="text-sm text-gray-600">
            {overviewStats.unassignedStores > 0 && (
              <span className="text-amber-600 whitespace-nowrap">
                {overviewStats.unassignedStores} loja{overviewStats.unassignedStores !== 1 ? 's' : ''} sem atribuição
              </span>
            )}
          </div>
        </div>
          ...
          return (
            <div 
              ...
              className={`bg-white rounded-xl shadow-sm border transition-all duration-200 hover:shadow-md ${
                isUnassigned ? 'border-amber-300 bg-amber-50' : 'border-gray-200'
              }`}
            >
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex items-start gap-3">
                    <div className={`h-10 w-10 lg:h-12 lg:w-12 rounded-full flex items-center justify-center flex-shrink-0 ${
                      ...
                      <Store className={`h-5 w-5 lg:h-6 lg:w-6 ${isUnassigned ? 'text-amber-600' : 'text-blue-600'}`} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="text-base lg:text-lg font-semibold text-gray-900 flex items-center flex-wrap gap-2">
                        <span className="truncate">{store.storeName}</span>
                        {isUnassigned && <AlertCircle className="h-4 w-4 text-amber-600 flex-shrink-0" />}
                      </h4>
                ...
                {!isUnassigned && (
                  <div className="mb-4">
                    <div className="flex justify-between text-sm text-gray-600 mb-2">
                      ...
                      <span>{store.totalSales} vendas • {store.clientsCount} clientes</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-3">
                      <div
                        className={`h-3 rounded-full transition-all duration-500 ${
                          store.conversionRate >= 70 ? 'bg-green-500' :
                          store.conversionRate >= 40 ? 'bg-yellow-500' : 'bg-red-500'
                        }`}
                        style={{ width: `${Math.min(store.conversionRate, 100)}%` }}
                      ></div>
                    </div>
                  </div>
                )}
                  ...
                  {isExpanded && !isUnassigned && (
                    <div className="mt-4 lg:mt-6 pt-4 lg:pt-6 border-t border-gray-200">
                      <div>
                        <h5 className="font-medium text-gray-900 mb-3">Informações Financeiras</h5>
                        <div className="space-y-3 lg:space-y-0 lg:grid lg:grid-cols-3 lg:gap-4">
                          <div className="flex justify-between lg:flex-col lg:text-center p-3 bg-gray-50 rounded-lg">
                            <span className="text-sm text-gray-600 lg:mb-1">Valor Total</span>
                            <span className="text-sm font-medium text-gray-900 truncate">{formatCurrency(store.totalAmount)}</span>
                          </div>
                          <div className="flex justify-between lg:flex-col lg:text-center p-3 bg-gray-50 rounded-lg">
                            <span className="text-sm text-gray-600 lg:mb-1">Eficiência</span>
                            <span className="text-sm font-medium text-green-600">
                              {store.totalAmount > 0 ? ((store.receivedAmount / store.totalAmount) * 100).toFixed(1) : 0}%
                            </span>
                          </div>
                          <div className="flex justify-between lg:flex-col lg:text-center p-3 bg-gray-50 rounded-lg">
                            <span className="text-sm text-gray-600 lg:mb-1">Ticket Médio</span>
                            <span className="text-sm font-medium text-blue-600 truncate">
                              {formatCurrency(store.totalSales > 0 ? store.totalAmount / store.totalSales : 0)}
                            </span>
                          </div>
                        </div>
                      </div>
                  ...
                  {isUnassigned && (
                    <div className="mt-4 p-3 lg:p-4 bg-amber-50 rounded-lg border border-amber-200">
                      <div className="flex items-center text-amber-700">
                        ...
                        <span className="text-sm font-medium">Esta loja não possui cobrador atribuído</span>
                      </div>
                    </div>
                  )}
                </div>
    ...
    {filteredAndSortedStores.length === 0 && (
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-600">
            {searchTerm ? 'Tente ajustar os filtros de busca.' : 'Não há lojas cadastradas no sistema.'}
          </p>
        </div>
      )}
    </div>
