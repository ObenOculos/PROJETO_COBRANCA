...
const ClientDetailModal: React.FC<ClientDetailModalProps> = ({ 
  ...
  const clientSales = React.useMemo(() => {
    ...
    console.log('Vendas do Cliente recalculadas:', sales.map(sale => ({
          ...
          {showClientData ? (
            <div className="overflow-y-auto max-h-[60vh] p-4 lg:p-6">
              ...
              <div className="space-y-3 text-sm text-gray-700">
                {Object.entries(clientGroup).filter(([key]) => 
                  ['client', 'document', 'address', 'number', 'neighborhood', 'city', 'phone', 'mobile'].includes(key)
                ).map(([key, value]) => {
                  ...
                  return (
                    <div key={key} className="flex justify-between items-center border-b pb-2 last:border-b-0 last:pb-0">
                      <span className="font-medium capitalize">{labelMap[key] || key.replace(/([A-Z])/g, ' $1').trim()}:</span>
                      <span className="text-right">{value ? (typeof value === 'object' && value !== null ? JSON.stringify(value) : String(value)) : '-'}</span>
                    </div>
                  );
                })}
              </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                              <div className="text-center p-3 bg-blue-50 rounded-lg">
                                <div className="text-base sm:text-lg font-bold text-blue-600">
                                  {formatCurrency(saleBalance.totalValue, false)}
                                </div>
