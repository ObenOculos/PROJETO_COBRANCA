...
const ClientDetailModal: React.FC<ClientDetailModalProps> = ({ 
  ...
  const clientSales = React.useMemo(() => {
    ...
    console.log('ClientSales recalculadas:', sales.map(sale => ({
          ...
          <div className="px-4 lg:px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <div className="flex items-center flex-1 min-w-0">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-sm text-gray-600 truncate">{clientGroup.document}</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="px-4 lg:px-6 py-4 bg-gray-50 border-b border-gray-200">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div className="text-center p-3 bg-blue-50 rounded-lg">
                <div className="text-lg font-bold text-blue-600">
                  {formatCurrency(clientSales.reduce((sum, sale) => sum + sale.totalValue, 0))}
                </div>
                <div className="text-xs text-gray-600">Valor Total</div>
              </div>
              <div className="text-center p-3 bg-green-50 rounded-lg">
                <div className="text-lg font-bold text-green-600">
                  {formatCurrency(clientSales.reduce((sum, sale) => sum + sale.totalReceived, 0))}
                </div>
                <div className="text-xs text-gray-600">Recebido</div>
              </div>
              <div className="text-center p-3 bg-red-50 rounded-lg">
                <div className="text-lg font-bold text-red-600">
                  {formatCurrency(clientSales.reduce((sum, sale) => sum + sale.pendingValue, 0))}
                </div>
            ...
            {userType === 'collector' && (
              <div className="mt-4 flex flex-col sm:flex-row justify-center gap-3">
                ...
                {clientSales.reduce((sum, sale) => sum + sale.pendingValue, 0) > 0 && (
                  <button
                    onClick={() => setIsGeneralPaymentModalOpen(true)}
                    className="flex items-center justify-center px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors font-semibold shadow-lg"
                ...
                {clientSales.reduce((sum, sale) => sum + sale.totalReceived, 0) > 0 && (
                  <button
                    onClick={() => setIsGeneralEditModalOpen(true)}
                    className="flex items-center justify-center px-6 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors font-semibold shadow-lg"
            ...
            <div className="mt-4 flex items-center justify-between">
              <div className="flex items-center text-sm text-gray-600">
                ...
                <span className="truncate">
                  {clientGroup.address}, {clientGroup.number} - {clientGroup.neighborhood}, {clientGroup.city}
                </span>
              </div>
          ...
          <div className="overflow-y-auto max-h-[60vh]">
            <div className="px-4 lg:px-6 py-4">
              <h3 className="text-lg font-medium text-gray-900 mb-4">
                Vendas ({clientSales.length} venda{clientSales.length !== 1 ? 's' : ''})
              </h3>
              ...
              <div className="space-y-3">
                {clientSales.map((sale) => {
                  ...
                  return (
                  <div key={sale.saleNumber} className="border border-gray-200 rounded-lg overflow-hidden">
                    {/* Sale Header */}
                    <div className="px-4 py-3 bg-gray-50">
                      <div className="flex items-center justify-between">
                        ...
                        >
                          ...
                          <div>
                            <h4 className="font-medium text-gray-900">Venda {sale.saleNumber}</h4>
                            ...
                            <div className="flex items-center mt-1">
                              <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                                ...
                                {saleBalance.status === 'fully_paid' ? 'Pago Integralmente' :
                                 saleBalance.status === 'partially_paid' ? 'Parcialmente Pago' :
                                 'Pendente'}
                              </span>
                            </div>
                          </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div className="text-center p-3 bg-green-50 rounded-lg">
                              ...
                              <div className="text-xs text-gray-600">Já Pago</div>
                            </div>
                            <div className="text-center p-3 bg-red-50 rounded-lg">
                              <div className="text-lg font-bold text-red-600">
                          ...
                          {userType === 'collector' && (
                            <div className="flex flex-col sm:flex-row gap-2 pt-3 border-t border-gray-200">
                              {saleBalance.remainingBalance > 0 && (
                                <button
                                  onClick={() => handleOpenSalePayment(sale)}
                              ...
                              <button
                                onClick={() => setIsGeneralEditModalOpen(true)}
                                className="flex items-center justify-center px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors font-medium"
                              >
                          ...
                          <div className="text-sm text-gray-600 pt-2 border-t border-gray-200">
                            <div className="flex justify-between items-center">
                              ...
                              <span className="font-medium">{sale.installments.length}</span>
                            </div>
                            <div className="flex justify-between items-center mt-1">
                              <span>Status da venda:</span>
                              <span className={`font-medium ${
                                saleBalance.status === 'fully_paid' ? 'text-green-600' :
                                saleBalance.status === 'partially_paid' ? 'text-yellow-600' :
                                'text-red-600'
                              }`}>
                                {saleBalance.status === 'fully_paid' ? 'Quitada' :
                                 saleBalance.status === 'partially_paid' ? 'Parcial' :
                                 'Pendente'}
                              </span>
                            </div>
                          </div>
