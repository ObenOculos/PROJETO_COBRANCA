...
const UpdateAddressModal: React.FC<UpdateAddressModalProps> = ({ isOpen, onClose, clientDocumento, onSuccess }) => {
  ...
  useEffect(() => {
    if (isOpen && clientDocumento) {
      const fetchCurrentAddress = async () => {
        ...
        } else if (error && error.code !== 'PGRST116') { // Ignore 'single row not found'
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Atualizar Endereço">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end space-x-2 pt-4">
          <button type="button" onClick={onClose} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none dark:bg-gray-600 dark:text-gray-100 dark:border-gray-500 dark:hover:bg-gray-700">
            Cancelar
          </button>
